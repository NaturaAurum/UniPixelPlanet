namespace UniPixelPlanet.Runtime
{
    public static class UniPixelPlanetShaderProps
    {
        public const string KeyPixels = "_Pixels";
        public const string KeyLightOrigin = "_Light_origin";
        public const string KeySeed = "_Seed";
        public const string KeyRotation = "_Rotation";
        public const string KeyTime = "time";
        public const string KeyCloudCover = "_Cloud_cover";

        public const string KeyGradientTex = "_GradientTex";
        public const string KeyTextureKeyword1 = "_ColorScheme";
        public const string KeyTextureKeyword2 = "_Dark_ColorScheme";

        public const string KeyColor1 = "_Color1";
        public const string KeyColor2 = "_Color2";
        public const string KeyColor3 = "_Color3";
        public const string KeyColor4 = "_Color4";

        public const string KeyBaseColor = "_Base_color";
        public const string KeyOutlineColor = "_Outline_color";
        public const string KeyShadowBaseColor = "_Shadow_base_color";
        public const string KeyShadowOutlineColor = "_Shadow_outline_color";

        public const string KeyColorRiver = "_River_color";
        public const string KeyColorRiverDark = "_River_color_dark";

        public const string KeyColorRiverCutoff = "_River_cutoff";

        public const string KeyColorBlack = "_Black_color";
    }
}